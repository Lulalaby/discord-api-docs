...
| waveform?      | string    | base64 encoded bytearray representing a [sampled waveform](#DOCS_RESOURCES_MESSAGE/attachment-object-visualizing-a-sampled-waveform) (currently for voice messages) |
...
###### Visualizing A Sampled Waveform

![Sample visualized waveform](sample-waveform.png)

<Collapsible title="Example javascript waveform visualizer" description="An example waveform visualizer written in javascript" icon="code">
An example `WaveformVisualizer` in javascript could look like this:

```javascript
class WaveformVisualizer {
  constructor(base64Waveform) {
    this.waveformByteData = this.decodeWaveform(base64Waveform);
  }

  // Decodes the base64 encoded waveform
  decodeWaveform(base64Waveform) {
    const binaryString = atob(base64Waveform);
    const byteData = new Uint8Array(binaryString.length);
  ...
  // Creates a waveform image
  createWaveformImage(width = 500, height = 100, backgroundColor = 'black', barColors = ['lightblue', 'lightskyblue', 'deepskyblue']) {
    ...
    const barWidth = width / this.waveformByteData.length / 2;
    const xScale = width / this.waveformByteData.length;
    const yScale = height / 2 / 255;

    // Create gradient
    const gradient = ctx.createLinearGradient(0, 0, 0, height);
    for (let i = 0; i < barColors.length; i++) {
      gradient.addColorStop(i / (barColors.length - 1), barColors[i]);
    }

    ctx.fillStyle = gradient;

    for (let i = 0; i < this.waveformByteData.length; i++) {
      const x1 = i * xScale;
      const barHeight = Math.max(2, this.waveformByteData[i] * yScale); // Ensure minimum height for small values
      const y1 = height / 2 - barHeight;
      const y2 = height / 2 + barHeight;

      // Draw the bars with rounded corners
      ctx.fillRect(x1, y1, barWidth, barHeight * 2);
    }
...
const base64Waveform = 'AAEABwkXExkIGFODWlJ6XFtMkV6UbzQQEhs=';
const visualizer = new WaveformVisualizer(base64Waveform);
const waveformCanvas = visualizer.createWaveformImage();
...
<Collapsible title="Example csharp waveform visualizer" description="An example waveform visualizer written in csharp" icon="code">
An example `WaveformVisualizer` in csharp could look like this.

You need to install the NuGet packages `Microsoft.Maui.Graphics` and `Microsoft.Maui.Graphics.Skia`.
...
public sealed class WaveformVisualizer
{
    ...
    private static byte[] DecodeWaveform(string base64Waveform)
      => return Convert.FromBase64String(base64Waveform);

    // Creates a waveform image
    public WaveformVisualizer CreateWaveformImage(string base64Waveform, int width = 500, int height = 100)
    {
        var waveformByteData = DecodeWaveform(base64Waveform);

        if (waveformByteData == null || waveformByteData.Length == 0)
            throw new ArgumentException("Waveform data is empty.");
        ...
        var barWidth = (float)width / waveformByteData.Length / 2;
        var xScale = (float)width / waveformByteData.Length;
        ...
        for (var i = 0; i < waveformByteData.Length; i++)
        {
            var x1 = i * xScale;
            var barHeight = Math.Max(2, waveformByteData[i] * yScale);
    ...
    // Saves the waveform image to a file
...
// Example usage
string base64Waveform = "AAEABwkXExkIGFODWlJ6XFtMkV6UbzQQEhs=";
await new WaveformVisualizer().CreateWaveformImage(base64Waveform, 500, 100).SaveImageAsync("waveform.png");
```

</Collapsible>

<Collapsible title="Example Python Waveform Visualizer" description="An example waveform visualizer written in Python" icon="code">
An example `WaveformVisualizer` in Python could look like this.

You need to install the python packages `numpy` and `Pillow`.
...
class WaveformVisualizer:
    def __init__(self, base64_waveform):
        self.waveform_byte_data = self.decode_waveform(base64_waveform)

    # Decodes the base64 encoded waveform
    def decode_waveform(self, base64_waveform):
        return np.frombuffer(base64.b64decode(base64_waveform), dtype=np.uint8)

    # Creates a waveform image
    def create_waveform_image(self, width=500, height=100, background_color=(0, 0, 0), bar_colors=None):
        if bar_colors is None:
            bar_colors = [(173, 216, 230), (135, 206, 235), (0, 191, 255)]  # LightBlue shades
        ...
        for i, value in enumerate(self.waveform_byte_data):
            x1 = i * x_scale
            bar_height = max(2, value * y_scale)  # Ensure minimum height for small values
...
# Example usage
base64_waveform = "AAEABwkXExkIGFODWlJ6XFtMkV6UbzQQEhs="
visualizer = WaveformVisualizer(base64_waveform)
waveform_image = visualizer.create_waveform_image(500, 100)
waveform_image.show()  # Display the image
waveform_image.save('waveform.png')  # Save the image to a file
```
